...
# Additional requirement which was not implemented
# Is that we need to identify the signals
# for entering the meeting
@b_thread
def meeting_sensor():
    while True:
        time.sleep(0.5)
        if random_choice(0.2):
            last_event = yield {request: BEvent("enter_meeting")}
            print("Entered meeting:", last_event)
            yield {waitFor: BEvent("exit_meeting")}


# Not operational
@b_thread
def audio_device_connected_sensor():
    while True:
        time.sleep(0.5)
        if random_choice(0.2):
            last_event = yield {request: BEvent("audio_device_connected")}
            print("audio_device_connected:", last_event)
            yield {waitFor: BEvent("audio_device_disconnected")}


# Not operational
def notify_on_sound_event():
    while True:
        last_event = yield {waitFor: BEvent("output_event")}
        yield {
            request: BEvent("show_notification", {"payload": last_event.data["data"]})
        }


# Not operational
def do_not_disturb_mode():
    while True:
        last_event = yield {waitFor: BEvent("enter_do_not_disturb")}
        yield {
            waitFor: BEvent("exit_do_not_disturb"),
            block: BEvent("show_notification"),
        }


# Not operational
# Also, open question how to implement?
def override_do_not_disturb_mode():
    while True:
        yield {waitFor: BEvent("enter_do_not_disturb")}
        yield {
            waitFor: BEvent("exit_do_not_disturb"),
            block: BEvent("show_notification"),
        }
...
if __name__ == "__main__":
    b_program = BProgram(
        bthreads=[audio_sensor(), odnn_scenario(), audio_actuator(), meeting_sensor()],
