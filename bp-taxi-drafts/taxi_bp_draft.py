...
There are additional hidden requirements here
'''
...
def taxi_dnn():
    while True:
        ...
        possible_output_events = dnn(state)
        yield { request: possible_output_events, block: AllInputEvents() }


'''
Req 2 - When the fuel_low light is on, 
        Refuel in the Red station, 
        And then proceed with the navigation to the passenger destination
'''
...
# Problem - if we allow non-determinism in the override of directions by the square
# The taxi could be stuck in a livelock 
# For example, if square (3,2) enables the output event up to reach the yellow destination
...
def forget_luggage_sensor():
    while True:
        ...
        yield { request: BEvent("forgot_luggage"), block: AllInputEvents() }
        while True:
...
def return_passenger_pickup_luggage():
    while True:
...
def scenario_1():
    while True:
        ...
        while True:
            yield { waitFor: BEvent("InputEvent", {"state": (0,4)}) }
            yield { request: BEvent("pickup_luggage") } 
            yield { request: BEvent("end_new_destination", {"dest": "green"} ) }
            break

# Requierment, there is a barrier in (1,3), and we need to avoid it.
    ...
    def scenario_2():
       while True:
                yield { waitFor: BEvent("InputEvent", {"state": (2,3)}) }
                yield { request: [BEvent("Left"),BEvent("Right")], block: [BEvent("Up")] } 
                break
